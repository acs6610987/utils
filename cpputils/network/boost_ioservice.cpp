...
#include <algorithm>
#include <sstream>
#include "badreceivebuffersize.h"
...
namespace network
{
    ...
    void BoostIOService::receiveOne(BoostSocket* socket)
    {
        ////////////////////////////////////////////////////////////////////////////////
        ...
        if (op.mType == BoostIOOperation::Type::RecvData)
        {
                ...
                [&op, socket, this](const boost::system::error_code& ec, uint64_t bytesTransfered)
            {
                ...
                {
                    Log::out << "recv error: " << ec.message() << Log::endl;
                    Log::out << "transmitted: " << bytesTransfered << "  out of " << boost::asio::buffer_size(op.mBuffs[0]) << Log::endl;
    ...
    void BoostIOService::sendOne(BoostSocket* socket)
    {
        ////////////////////////////////////////////////////////////////////////////////
    ...
    void BoostIOService::dispatch(BoostSocket* socket, BoostIOOperation& op)
    {
        if (&op == nullptr)
        {
            Log::out << "bad op ref";
        ...
        case BoostIOOperation::Type::SendData:
            ...
            if (op.mSize == 0)
                throw std::runtime_error("Networking error: attempted to send zero sized message");
    ...
    BoostAcceptor* BoostIOService::getAcceptor(BoostEndpoint& endpoint)
    {
        ...
        if (endpoint.isHost())
        {
            ...
            auto acceptorIter = std::find_if(
                mAcceptors.begin(),
                mAcceptors.end(), [&](const BoostAcceptor& acptr)
            {
