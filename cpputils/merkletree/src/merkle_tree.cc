...
// Zico: leaf is indexed from 1. We can see this from the function LeafHash(size_t leaf)
...
std::vector<string> MerkleTree::SnapshotConsistency(size_t snapshot1,
  ...
  // Compute the (compressed) path to the root of snapshot2 (Zico: snapshot1?).
...
string MerkleTree::UpdateToSnapshot(size_t snapshot) {
  if (snapshot == 0)
    return treehasher_.HashEmpty();
  if (snapshot == 1) // Zico: but this does not update the root, right?
    return Node(0, 0);
  if (snapshot == leaves_processed_)
    return Root();
...
/* The existence of the param 'node' is so weird and annoying. Basically,
 it just records the last node on 'node_level' for the Merkle tree of 'snapshot'.
 But it has nothing to do with the functionality of this function 'RecomputePastSnapshot'.
 Why do you need to record the last node of a specific level??? (it's used in the function PathFromNodeToRootAtSnapshot...) */
string MerkleTree::RecomputePastSnapshot(size_t snapshot, size_t node_level,
  ...
  // Index of the rightmost node at the current level for this snapshot.
    // Zico: The current level is 0, but last_node could change as we will see below.
  size_t last_node = snapshot - 1;

  if (snapshot == leaves_processed_) {
    // Nothing to recompute.
    if (node && LazyLevelCount() > node_level) {
      if (node_level > 0) { // Zico: Why separate the two conditions? Because on level 0, there could be some leaves (after snapshot) that have not been integrated into the Merkle tree structure.
  ...
  while (MerkleTreeMath::IsRightChild(last_node)) {
    ...
    // Zico: why do we need to find a left sibling?
    // Because that's when recomputation needs to begin. In a perfect scenario, if snapshot is a power of 2, we don't actually need to recompute anything, because the Merkle tree of this snapshot is just a subtree of the current tree.
...
std::vector<string> MerkleTree::PathFromNodeToRootAtSnapshot(size_t node,
  ...
  // Zico: Because siblings are what constitue the Merkle verification path
  while (last_node) {
